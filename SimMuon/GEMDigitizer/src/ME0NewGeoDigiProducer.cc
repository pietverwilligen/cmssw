...
#include "SimMuon/GEMDigitizer/interface/ME0NewGeoDigiProducer.h"
...
ME0NewGeoDigiProducer::TemporaryGeometry::TemporaryGeometry(const ME0Geometry* geometry, const unsigned int numberOfStrips, const unsigned int numberOfPartitions) {
...
//	}
	...
	for(unsigned int iP = 0; iP < numberOfPartitions; ++iP){
		...
		LogDebug("ME0NewGeoDigiProducer::TemporaryGeometry") << "Top of new partition: " <<partionTops.back() << std::endl;
	}
	...
	tofs.resize(nLayers);
	LogDebug("ME0NewGeoDigiProducer::TemporaryGeometry") << "TOF numbers [layer][partition]: " ;
	for(unsigned int iL = 0; iL < nLayers; ++iL){
...
unsigned int ME0NewGeoDigiProducer::TemporaryGeometry::findEtaPartition(float locY) const {
...
float ME0NewGeoDigiProducer::TemporaryGeometry::getPartCenter(const unsigned int partIdx) const {return stripTopos[partIdx]->radius() - middleDistanceFromBeam ;}

ME0NewGeoDigiProducer::TemporaryGeometry::~TemporaryGeometry() {
	for(auto * p : stripTopos) { delete p;}
}

TrapezoidalStripTopology * ME0NewGeoDigiProducer::TemporaryGeometry::buildTopo(const std::vector<float>& _p) const {
	float b = _p[0];
	float B = _p[1];
	float h = _p[2];
	float r0 = h*(B + b)/(B - b);
	...
	LogDebug("ME0NewGeoDigiProducer::TemporaryGeometry") << "New partition parameters: " <<
...
ME0NewGeoDigiProducer::ME0NewGeoDigiProducer(const edm::ParameterSet& ps) :
		...
		token(consumes<ME0DigiPreRecoCollection>(edm::InputTag(ps.getParameter<std::string>("inputCollection"))))
{
	...
	if (!rng.isAvailable()){
		...
		<< "ME0NewGeoDigiProducer::ME0PreRecoDigiProducer() - RandomNumberGeneratorService is not present in configuration file.\n"
	...
	if(numberOfSrips == 0)
		throw cms::Exception("Setup") << "ME0NewGeoDigiProducer::ME0PreRecoDigiProducer() - Must have at least one strip.";
	if(numberOfPartitions == 0)
		throw cms::Exception("Setup") << "ME0NewGeoDigiProducer::ME0PreRecoDigiProducer() - Must have at least one partition.";
	if(neutronAcceptance < 0 )
		throw cms::Exception("Setup") << "ME0NewGeoDigiProducer::ME0PreRecoDigiProducer() - neutronAcceptance must be >= 0.";
...
ME0NewGeoDigiProducer::~ME0NewGeoDigiProducer()
{
	if(tempGeo) delete tempGeo;
}


void ME0NewGeoDigiProducer::beginRun(const edm::Run&, const edm::EventSetup& eventSetup)
{
	// set geometry
...
void ME0NewGeoDigiProducer::produce(edm::Event& e, const edm::EventSetup& eventSetup)
{
	edm::Service<edm::RandomNumberGenerator> rng;
...
void ME0NewGeoDigiProducer::buildDigis(const ME0DigiPreRecoCollection & input_digis,
		ME0DigiPreRecoCollection & output_digis,
		ME0DigiPreRecoMap & output_digimap,
		CLHEP::HepRandomEngine* engine)
{

	LogDebug("ME0NewGeoDigiProducer::buildDigis") << "Begin bulding digis."<<std::endl;
	...
	for (me0dgIt = input_digis.begin(); me0dgIt != input_digis.end();
		...
		LogTrace("ME0NewGeoDigiProducer::buildDigis") << "Starting with chamber: "<< me0Id<<std::endl;
		...
		for (ME0DigiPreRecoCollection::const_iterator digi = range.first;
			...
			LogTrace("ME0NewGeoDigiProducer::buildDigis") << partIdx <<" ";
			...
			LogTrace("ME0NewGeoDigiProducer::buildDigis") << tof <<"("<<bxIdx<<") ";
			...
			LogTrace("ME0NewGeoDigiProducer::buildDigis") << "("<<bxIdx<<","<<partIdx<<","<<strip<<") ";
			...
			LogTrace("ME0NewGeoDigiProducer::buildDigis") << "("<<digiChamberLocalPoint.x()<<","<<digiChamberLocalPoint.y()<<","<<sigmaX<<","<<sigmaY<<","<< tof<<") ";
		}
