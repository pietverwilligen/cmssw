...
void RPCSimSetUp::setRPCSetUp(const std::vector<RPCStripNoises::NoiseItem>& vnoise, const std::vector<RPCClusterSize::ClusterSizeItem>& vClusterSize){
  ...
  // std::cout<<"RPCSimSetUp::setRPCSetUp(vector<NoiseItem>, vector<ClusterSizeItem>)"<<std::endl;
  ...
  std::vector<double> clsVect;
  for(itCls = vClusterSize.begin(); itCls != vClusterSize.end(); ++itCls){
    // std::cout<<" Push back clustersize = "<<itCls->clusterSize<<std::endl;
    clsVect.push_back(((double)(itCls->clusterSize)));
    // std::cout<<"Filling cls in _mapDetClsMapLegacy[detId,clsVect] :: detId = "<<detId<<" --> will it be accepted? clsCounter = "<<clsCounter<<" accepted? "<<((!(clsCounter%120)) && (clsCounter!=0)); // <<std::endl;
    if((!(clsCounter%100)) && (clsCounter!=0)){
      detId=itCls->dpid;
      _mapDetClsMapLegacy[detId]=clsVect;
      clsVect.clear();
      // std::cout<<" --> saved in map "<<std::endl;
      // std::cout<<"Filling cls in _mapDetClsMapLegacy[detId,clsVect] :: detId = "<<detId<<" --> will it be accepted? clsCounter = "<<clsCounter<<" accepted? "<<((!(clsCounter%120)) && (clsCounter!=0))<<std::endl;
      clsCounter=0;
    }
    /*
    else{
      std::cout<<" --> not saved in map "<<std::endl;
    }
    */
    ++clsCounter;
  }
...
const std::vector<float>& RPCSimSetUp::getNoise(uint32_t id)
{
  ...
  // std::cout<< "All OK coming from RPCSimSetUp - noise information for DetId\t"<<id<< std::endl;
  return iter->second;
}
...
const std::vector<double>& RPCSimSetUp::getCls(uint32_t id) //legacy member function
{
  ...
  std::cout<<"RPCSimSetUp::getCls"<<std::endl;
  ...
  std::cout<< "All OK coming from RPCSimSetUp - _mapDetClsMapLegacy - cluster size information for DetId\t"<<id<< std::endl;
  return iter->second;
}

const std::vector<double> & RPCSimSetUp::getAsymmetricClsDistribution(uint32_t id, uint32_t slice){

  std::cout<<"RPCSimSetUp::getAsymmetricClsDistribution"<<std::endl;
...
const std::vector<double> & RPCSimSetUp::getAsymmetryForCls(uint32_t id, uint32_t slice, uint32_t cls){
  ...
  std::cout<<"RPCSimSetUp::getAsymmetryForCls"<<std::endl;
