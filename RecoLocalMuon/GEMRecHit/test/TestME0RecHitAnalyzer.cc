// -*- C++ -*-
//
// Package:    TestME0RecHitAnalyzer
// Class:      TestME0RecHitAnalyzer
// 
/**\class TestME0RecHitAnalyzer TestME0RecHitAnalyzer.cc MyAnalyzers/TestME0RecHitAnalyzer/src/TestME0RecHitAnalyzer.cc
...
class TestME0RecHitAnalyzer : public edm::EDAnalyzer {
   public:
      explicit TestME0RecHitAnalyzer(const edm::ParameterSet&);
      ~TestME0RecHitAnalyzer();
   ...
   private:
      ...
      // ----------member data ---------------------------
  edm::ESHandle<ME0Geometry> gemGeom;
...
//
TestME0RecHitAnalyzer::TestME0RecHitAnalyzer(const edm::ParameterSet& iConfig)
...
TestME0RecHitAnalyzer::~TestME0RecHitAnalyzer()
{
...
void
TestME0RecHitAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  // ME0 recHits
  // ================
  edm::Handle<ME0RecHitCollection> gemRecHits; 
  iEvent.getByToken(ME0RecHit_Token,gemRecHits);

  // count the number of ME0 rechits
  int nME0 = 0;
  ME0RecHitCollection::const_iterator recHit;
  for (recHit = gemRecHits->begin(); recHit != gemRecHits->end(); recHit++) {
    nME0++;
  }
   
  // std::cout<<"The Number of RecHits is "<<nME0<<std::endl;       
  for (recHit = gemRecHits->begin(); recHit != gemRecHits->end(); recHit++) {
    ME0DetId rollId = (ME0DetId)(*recHit).me0Id();
    LocalPoint recHitPos=recHit->localPosition();
    const ME0EtaPartition* rollasociated = gemGeom->etaPartition(rollId);
    const BoundPlane & ME0Surface = rollasociated->surface(); 
    GlobalPoint ME0GlobalPoint = ME0Surface.toGlobal(recHitPos);
    ...
    std::cout<<"ME0 Rec Hit in [DetId] = ["<<rollId<<"] with BX = "<<recHit->tof()<<" and Global Position = "<<ME0GlobalPoint<<std::endl;
    ...
    if(region == 1 && station == 1) {
      x_p1.push_back(ME0GlobalPoint.x()); y_p1.push_back(ME0GlobalPoint.y()); z_p1.push_back(ME0GlobalPoint.z()); r_p1.push_back(sqrt(pow(ME0GlobalPoint.x(),2) + pow(ME0GlobalPoint.y(),2)));
      // Layers
      if(layer==1) { BX_RecHits_GE1in_Plus->Fill(bx);  ST_RecHits_GE1in_Plus->Fill(st); CL_RecHits_GE1in_Plus->Fill(cl); x_p1i.push_back(ME0GlobalPoint.x()); y_p1i.push_back(ME0GlobalPoint.y()); z_p1i.push_back(ME0GlobalPoint.z()); 
	r_p1i.push_back(sqrt(pow(ME0GlobalPoint.x(),2) + pow(ME0GlobalPoint.y(),2)));}
      if(layer==2) { BX_RecHits_GE1out_Plus->Fill(bx); ST_RecHits_GE1out_Plus->Fill(st); CL_RecHits_GE1out_Plus->Fill(cl); x_p1o.push_back(ME0GlobalPoint.x()); y_p1o.push_back(ME0GlobalPoint.y()); z_p1o.push_back(ME0GlobalPoint.z());
	r_p1o.push_back(sqrt(pow(ME0GlobalPoint.x(),2) + pow(ME0GlobalPoint.y(),2)));}
    }
    // Negative Endcap
    if(region == -1 && station == 1) {
      x_n1.push_back(ME0GlobalPoint.x()); y_n1.push_back(ME0GlobalPoint.y()); z_n1.push_back(ME0GlobalPoint.z()); r_n1.push_back(sqrt(pow(ME0GlobalPoint.x(),2) + pow(ME0GlobalPoint.y(),2)));
      // Layers
      if(layer==1) { BX_RecHits_GE1in_Minus->Fill(bx);  ST_RecHits_GE1in_Minus->Fill(st);  CL_RecHits_GE1in_Minus->Fill(cl); x_n1i.push_back(ME0GlobalPoint.x()); y_n1i.push_back(ME0GlobalPoint.y()); z_n1i.push_back(ME0GlobalPoint.z());
	r_n1i.push_back(sqrt(pow(ME0GlobalPoint.x(),2) + pow(ME0GlobalPoint.y(),2)));}
      if(layer==2) { BX_RecHits_GE1out_Minus->Fill(bx); ST_RecHits_GE1out_Minus->Fill(st); CL_RecHits_GE1out_Minus->Fill(cl); x_n1o.push_back(ME0GlobalPoint.x()); y_n1o.push_back(ME0GlobalPoint.y()); z_n1o.push_back(ME0GlobalPoint.z());
	r_n1o.push_back(sqrt(pow(ME0GlobalPoint.x(),2) + pow(ME0GlobalPoint.y(),2)));}
    }
  }
}
...
void
TestME0RecHitAnalyzer::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
DEFINE_FWK_MODULE(TestME0RecHitAnalyzer);
